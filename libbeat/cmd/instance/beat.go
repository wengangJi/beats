...
type beatConfig struct {
	...
	// instance internal configs 1111
...
func (b *Beat) createBeater(bt beat.Creator) (beat.Beater, error) {
	...
	//修改pipeline，为不影响性能多output使用同一通道，在创建队列输出时增加多outworker
	pipeline, err := pipeline.Loads(b.Info,
		pipeline.Monitors{
			Metrics:   reg,
		...
		b.Config.Pipeline,
		b.processing,
		b.makeOutputFactorys(b.Config.Output),
	)
...
//修改outputFactory 函数，增加多路输出[]output.group
func (b *Beat) makeOutputFactorys(
	cfg common.ConfigNamespace,
) func(outputs.Observer) (string, []outputs.Group, error) {
	return func(outStats outputs.Observer) (string, []outputs.Group, error) {
		out, err := b.createOutputs(outStats, cfg)
		return cfg.Name(), out, err
	}
}


//修改创建多路输出group,根据配置文件多个out初始化
func (b *Beat) createOutputs(stats outputs.Observer, cfg common.ConfigNamespace) ([]outputs.Group, error) {
	if !cfg.IsSet() {
		return []outputs.Group{}, nil
	}
	var groups []outputs.Group
	for _,n := range  cfg.Names() {
		group,err := outputs.Load(b.index, b.Info, stats, n, cfg.Configs(n))
		if err != nil {
			return nil,err
		}
...
func (b *Beat) makeOutputReloader(outReloader pipeline.OutputReloader) reload.Reloadable {
	return reload.ReloadableFunc(func(config *reload.ConfigWithMeta) error {
		return outReloader.Reload(config, b.createOutputs)
	})
}
