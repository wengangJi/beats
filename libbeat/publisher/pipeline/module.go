...
//修改output结构体，输出多路output
type OutputFactorys func(outputs.Observer) (string, []outputs.Group, error)
...
// 修改初始化pipeline初始化加载函数，创建多路output输出对象，输出pipeline实例化instance
func Loads(
	beatInfo beat.Info,
	monitors Monitors,
	config Config,
	processors processing.Supporter,
	makeOutputs func(outputs.Observer) (string, []outputs.Group, error),
) (*Pipeline, error) {
	log := monitors.Logger
	if log == nil {
		log = logp.L()
	...
	out, err := loadOutputs(monitors, makeOutputs)
	if err != nil {
		return nil, err
	}
...
// 修改多路输出output加载函数，增加outputfactorys方法，根据namespace创建output
func loadOutputs(
	monitors Monitors,
	makeOutput OutputFactorys,
) ([]outputs.Group, error) {
	log := monitors.Logger
	if log == nil {
		log = logp.L()
