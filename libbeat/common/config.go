...
func (ns *ConfigNamespace) Names() []string {
	var names []string
	for n := range ns.C {
...
func (ns *ConfigNamespace) Configs(key string) *Config {
	if config, ok := ns.C[key]; ok {
		return config
	}
	return nil
}
