...
func (h *Harvester) Run() error {
	...
	for {
		...
		//为兼容版本6 输出不影响线上日志解析使用，修改event输出内容
		//此处修改只针对log input 类型，其余input可保持现状，此处修改主要与线上解析同步
		// Check if data should be added to event. Only export non empty events.
		//log 改为logs ,用于打点日志使用log关键字。 log关键字解析获取日志ID值以及decode_json_fields通过log关键字为false执行
		if !message.IsEmpty() && h.shouldExportLine(text) {
			...
			if h.config.JSON != nil && len(jsonFields) > 0 {
				...
				// 该字段为公共字段，不可变更，通过MergeJSONFields 做decode_json_fields。
				fields["message"] = text


			}

			// 为兼容打点日志字段统一，增加source字段输出
			fields["source"] = state.Source
